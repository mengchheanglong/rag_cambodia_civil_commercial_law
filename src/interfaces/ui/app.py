...
with st.sidebar:
    ...
    top_k = st.slider("Top Articles to Retrieve:", min_value=1, max_value=10, value=4)
    ...
    st.info(
        "💡 **Tip**: Queries can be conceptual (e.g. *'What is a defect in sold goods?'*) or exact (e.g. *'Article 315'*)."
    )
...
with tab_qa:
    ...
    example_questions = [
        "How is a contract formed under the Cambodian Civil Code?",
        ...
        "What happens if a buyer delays paying the purchase price?",
    ]
    ...
    if st.button("🚀 Analyze & Answer", type="primary"):
        if not user_query.strip():
            ...
            with st.spinner("Retrieving relevant articles and generating grounded analysis..."):
                try:
                    ...
                    # If OpenAI API key is not configured, show retrieval results
...
with tab_search:
    ...
    search_term = st.text_input(
        ...
        placeholder="e.g. good faith, arbitration, Article 315",
    )
